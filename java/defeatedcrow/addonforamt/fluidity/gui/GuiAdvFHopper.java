...
public class GuiAdvFHopper extends GuiContainer {
	...
	public GuiAdvFHopper(EntityPlayer thisPlayer, TileAdvFluidHopper hopper) {
		super(new ContainerAdvFHopper(thisPlayer, hopper));
		this.tile = hopper;
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		if (b3 && tile.getFilterFluid() != null) {
			...
			list3.add("Filter : "
					+ this.tile.getFilterFluid().getLocalizedName(new FluidStack(this.tile.getFilterFluid(), 1000)));
	...
	protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
		...
		this.mc.getTextureManager().bindTexture(
				new ResourceLocation("fluiditydc", "textures/gui/fluidhopper_adv_gui.png"));
	...
	protected void mouseClicked(int x, int y, int button) {
		...
		if (ix > 0 && ix < this.xSize && iy > 0 && iy < this.ySize / 2) {
			...
			if (ix > 79 && ix < 95 && iy > 61 && iy < 76) {
				...
				return;
			} else if (ix > 99 && ix < 115 && iy > 61 && iy < 76 && player.inventory.getItemStack() == null) {
				MessageHopperFilter message = new MessageHopperFilter(0, tile.xCoord, tile.yCoord, tile.zCoord);
				NetworkHandlerFF.INSTANCE.sendToServer(message);
			}
		}
