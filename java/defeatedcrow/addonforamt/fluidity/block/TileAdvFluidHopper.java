 ...
 */
public class TileAdvFluidHopper extends TileEntity implements IFluidHandler, ISidedInventory {
	...
	private boolean canActivate() {
		...
		{
			if (this.filterFluid == 0)
				return false;
			else if (!this.productTank.isEmpty() && this.productTank.getFluidType().getID() != this.filterFluid)
				return false;
	...
	// モード設定
	public enum Mode {
		Entity(1), FluidType(2), RedStone(3), Default(0);
		...
		public static final Mode[] ModeList = {
				Default,
				Entity,
				FluidType,
				RedStone };
